...
const STORAGE_KEY = "lido-final-status-v1";
const NOTE_KEY = "lido-final-notes-v1";
const states = ["free", "occupied", "reserved", "maintenance"];

let savedStatus = JSON.parse(localStorage.getItem(STORAGE_KEY) || "{}");
let savedNotes = JSON.parse(localStorage.getItem(NOTE_KEY) || "{}");
let status = JSON.parse(JSON.stringify(savedStatus));
let notes = JSON.parse(JSON.stringify(savedNotes));
let selectedId = null;
let dirty = false;
...
function updateSaveStatus(justSaved=false) {
  saveStatusEl.classList.remove("dirty", "saved");

  if (justSaved) {
    const now = new Date().toLocaleTimeString([], {hour:"2-digit", minute:"2-digit", second:"2-digit"});
    saveStatusEl.textContent = `✓ Layout saved at ${now}`;
    saveStatusEl.classList.add("saved");
    return;
  }

  if (dirty) {
    saveStatusEl.textContent = "Unsaved changes — press Salva.";
...
function save() {
  ...
  savedNotes = JSON.parse(JSON.stringify(notes));
  dirty = false;
  updateSaveStatus(true);
}

function undo() {
  status = JSON.parse(JSON.stringify(savedStatus));
...
document.getElementById("undoBtn").onclick = () => {
  if (!dirty) return;
  if (confirm("Annullare le modifiche non salvate?")) undo();
};
